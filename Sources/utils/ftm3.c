 ...
 * \date          03.04.20018
 *
 * $Id: ftm3.c 104 2018-05-01 14:07:11Z zajost $
 *
 *--------------------------------------------------------------------
 */
...
void ftm3Init(void)
{
  // todo #9.01 set clockgating for FTM3
  SIM_SCGC6 |= SIM_SCGC6_FTM3(1);

  // sets the modulo
  FTM3_MOD = FTM3_MODULO;

  // todo #9.02 configure the timer with "system clock" as clocksource and with a "Prescaler" of 1 => 60 MHz
  FTM3_SC |= FTM_SC_CLKS(1) | FTM_SC_PS(0);

  // todo #9.03 Enable FTM3 interrupt on NVIC with Prio: PRIO_FTM0 (defined in platform.h)
  NVIC_EnableIRQ(FTM3_IRQn);
  NVIC_SetPriority(FTM3_IRQn, PRIO_FTM0);
