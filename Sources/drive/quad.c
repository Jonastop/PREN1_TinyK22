 ...
 * \date          03.04.20018
 *
 * $Id: quad.c 87 2018-04-17 09:32:21Z zajost $
 *
 *--------------------------------------------------------------------
 */
...
#define WHEEL_DIAMETER            110      // 120 mm
...
void FTM1_IRQHandler(void)
{
  ...
  if (FTM1_SC & FTM_SC_TOF_MASK) {                  // check for timer overflow
    FTM1_SC &= ~FTM_SC_TOF_MASK;                    // Clear tof flag
  ...
  if (FTM1_C0SC & FTM_CnSC_CHF_MASK) chValue = FTM1_C0V;  // determine the channel chValue of this interrupt (CH0 or CH1)
  if (FTM1_C1SC & FTM_CnSC_CHF_MASK) chValue = FTM1_C1V;
  ...
  if (FTM1_C0SC & FTM_CnSC_CHF_MASK) {      // check for channel 0 interrupt
    FTM1_C0SC &= ~FTM_CnSC_CHF_MASK;        // clear interrupt flag
  ...
  if (FTM1_C1SC & FTM_CnSC_CHF_MASK) {      // check for channel 1 interrupte
    FTM1_C1SC &= ~FTM_CnSC_CHF_MASK;        // clear interrupt fla
...
void quadInit(void)
{
  ...
  PORTA_PCR12 = PORT_PCR_MUX(3) | PORT_PCR_PE(1) | PORT_PCR_PS(1);
  PORTA_PCR13 = PORT_PCR_MUX(3) | PORT_PCR_PE(1) | PORT_PCR_PS(1);
  ...
  FTM1_SC = FTM_SC_CLKS(2) |  FTM_SC_PS(0) | FTM_SC_TOIE(1);
  ...
  FTM1_C0SC = FTM_CnSC_CHIE(1) | FTM_CnSC_MSx(0) | FTM_CnSC_ELSx(3) | FTM_CnSC_ICRST(1);
  FTM1_C1SC = FTM_CnSC_CHIE(1) | FTM_CnSC_MSx(0) | FTM_CnSC_ELSx(3) | FTM_CnSC_ICRST(1);
